...
export default function ThankYouPage() {
  ...
  useEffect(() => {
    ...
    if (!purchaseTracked) {
      const alreadyTracked = sessionStorage.getItem('fb_purchase_tracked_es');
      if (!alreadyTracked) {
        ...
        const trackPurchase = () => {
          ...
          // Usa i dati salvati dalla landing o fallback a valori di default ES
          const eventData = storedEventData || {
            value: 69,
            currency: 'EUR',
            content_name: 'Product ES',
            content_type: 'product' as const,
            content_ids: 'product-es'
          };

          console.log('[TY-ES] Using event data:', eventData);
          trackPurchaseEvent(userData, eventData);

          sessionStorage.setItem('fb_purchase_tracked_es', 'true');
          setPurchaseTracked(true);
          console.log('[TY-ES] Facebook Purchase tracked');
        };
    ...
    <div className="ty-container" style={{
      ...
      <div className="ty-box" style={{
        ...
        <div style={{
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: '0.75rem', marginBottom: '1rem' }}>
            ...
            <div>
              ...
              <div style={{ color: '#64748b', fontSize: '0.85rem' }}>Pagarás en efectivo al repartidor</div>
            </div>
          </div>
          <div style={{ display: 'flex', alignItems: 'center', gap: '0.75rem', marginBottom: '1rem' }}>
            ...
            <div>
              <div style={{ fontWeight: 600, color: '#1E293B', fontSize: '0.95rem' }}>Envío gratis</div>
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: '0.75rem', marginBottom: '1rem' }}>
            ...
            <div>
              ...
              <div style={{ color: '#64748b', fontSize: '0.85rem' }}>Reembolso si no estás satisfecho</div>
            </div>
          </div>
        ...
        <div style={{
          ...
          <div style={{ fontSize: '0.8rem', color: '#94a3b8', marginTop: '0.5rem' }}>Puedes solicitar la devolución en 30 días si no estás satisfecho con el producto</div>
        </div>
