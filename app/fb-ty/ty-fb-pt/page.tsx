  ...
  useEffect(() => {
    ...
    if (!purchaseTracked) {
      ...
      if (skipFBPurchase === 'true') {
        console.log('[TY-PT] Skipping FB Purchase - DOUBLE from network');
        sessionStorage.removeItem('skipFBPurchase');
        setPurchaseTracked(true);
        return;
      }

      const alreadyTrackedFb = sessionStorage.getItem('fb_purchase_tracked_pt');
      if (!alreadyTrackedFb) {
        const trackPurchase = () => {
          ...
          const eventData = storedEventData || {
            ...
            content_ids: 'quickchef-pt'
          };

          console.log('[TY-PT] Using event data:', eventData);
          trackPurchaseEvent(userData, eventData);

          sessionStorage.setItem('fb_purchase_tracked_pt', 'true');
          setPurchaseTracked(true);
          console.log('[TY-PT] Facebook Purchase tracked');
        };
    ...
    <div className="ty-container" style={{
      ...
      <div className="ty-box" style={{
        ...
        <div style={{
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: '0.75rem', marginBottom: '1rem' }}>
            ...
            <div>
              ...
              <div style={{ color: '#64748b', fontSize: '0.85rem' }}>Paga em dinheiro ao estafeta</div>
            </div>
          </div>
          <div style={{ display: 'flex', alignItems: 'center', gap: '0.75rem', marginBottom: '1rem' }}>
            ...
            <div>
              <div style={{ fontWeight: 600, color: '#1E293B', fontSize: '0.95rem' }}>Envio grátis</div>
              <div style={{ color: '#64748b', fontSize: '0.85rem' }}>Entrega em 24-48h</div>
            </div>
          </div>
          <div style={{ display: 'flex', alignItems: 'center', gap: '0.75rem', marginBottom: '1rem' }}>
            ...
            <div>
              ...
              <div style={{ color: '#64748b', fontSize: '0.85rem' }}>Reembolso se não ficar satisfeito</div>
            </div>
          </div>
        ...
        <div style={{
          ...
          <div style={{ fontSize: '0.85rem', color: '#64748b', marginBottom: '0.25rem' }}>Precisa de ajuda ou quer fazer uma devolução?</div>
          <a href="mailto:info@ionizi.com" style={{ color: '#FF8C00', fontWeight: 600, textDecoration: 'none', fontSize: '0.95rem' }}>info@ionizi.com</a>
          <div style={{ fontSize: '0.8rem', color: '#94a3b8', marginTop: '0.5rem' }}>Pode pedir o reembolso em 30 dias se não ficar satisfeito com o produto</div>
        </div>
