...
// Network config for SL (Facebook Uncapped)
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if(formState.name && formState.phone && formState.address) {
        ...
        try {
          ...
          window.location.href = '/fb-ty/ty-fb-sl';
        } catch (error) {
      ...
      return (
          <section id="order-form" className="py-16 px-4 bg-green-50 min-h-[50vh] flex items-center justify-center">
              <div className="max-w-md w-full bg-white p-8 rounded-2xl shadow-2xl text-center border-4 border-green-500">
                  ...
                  <p className="text-xl text-gray-700 mb-6">Vaše naročilo je potrjeno.</p>
                  <p className="text-gray-600">Naš agent vas bo kmalu kontaktiral na številko <span className="font-bold">{formState.phone}</span> glede dostave.</p>
                  <div className="mt-6 bg-yellow-100 p-4 rounded-lg border border-yellow-300">
                    <p className="font-bold text-gray-800 uppercase text-sm mb-1">Znesek za plačilo kurirju:</p>
    ...
    <section id="order-form" className="py-12 px-4 bg-gray-200">
      <div className="max-w-2xl mx-auto bg-white rounded-2xl shadow-2xl overflow-hidden border-2 border-gray-300">
        ...
        <div className="bg-red-600 text-white p-6 text-center">
          <h2 className="text-3xl font-black uppercase mb-2">OBRAZEC ZA NAROČILO</h2>
          <p className="text-lg font-medium">Izpolnite spodaj za rezervacijo promocije</p>
          ...
          <div className="flex items-center justify-center gap-2 mt-4 bg-red-700 py-2 rounded-lg border border-red-500">
            ...
            <span className="font-bold text-xl">Ponudba poteče čez: {formatTime(timeLeft)}</span>
          </div>
        </div>
        ...
        <div className="p-6 md:p-8">
          ...
          <div className="bg-yellow-50 border-2 border-yellow-400 p-4 rounded-lg mb-8 shadow-sm">
            <h3 className="font-bold text-gray-800 text-lg mb-2 border-b border-yellow-200 pb-2">Pregled naročila:</h3>
            ...
            <div className="flex justify-between items-center mt-3 pt-3 border-t-2 border-yellow-200 text-2xl font-black text-red-600">
              <span>SKUPAJ (Plačilo ob prevzemu):</span>
          ...
          <form onSubmit={handleSubmit} className="space-y-6">
            ...
            <div>
              <label className="block text-gray-700 font-bold mb-2 text-lg">Telefonska številka <span className="text-red-500">*</span></label>
              <input
                type="tel"
                name="phone"
                required
                placeholder="Npr. 031 123 456"
                className="w-full p-4 border-2 border-gray-300 rounded-lg text-lg focus:border-green-500 focus:ring-green-500 outline-none"
                value={formState.phone}
                onChange={handleChange}
              />
              <p className="text-sm text-gray-500 mt-1 font-medium bg-blue-50 p-2 rounded text-blue-800">ℹ️ Pomembno: Kurir vas bo poklical na to številko pred dostavo.</p>
            </div>

            <div>
              <label className="block text-gray-700 font-bold mb-2 text-lg">Naslov za dostavo <span className="text-red-500">*</span></label>
              <textarea
                name="address"
                required
                placeholder="Mesto, Ulica, Hišna številka, Nadstropje..."
                rows={3}
            ...
            <div className="bg-gray-100 p-4 rounded-lg flex items-center gap-3 text-sm text-gray-600 border border-gray-200">
               ...
               <span>Brez tveganja: plačate z gotovino šele, ko prejmete paket.</span>
            </div>
            ...
            >
              {isSubmitting ? 'POŠILJAM...' : 'POTRDI NAROČILO'}
              <span className="block text-lg font-normal mt-2 text-green-100">Hitra Dostava in Plačilo ob Prevzemu</span>
            </button>
          </form>
        </div>
      </div>
